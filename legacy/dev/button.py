...
async def vote(ctx):
    ...
    async def on_timeout():
        ...
        embed.set_author(name=ctx.message.author, icon_url=ctx.message.author.avatar_url)  # Botのアイコンを設定してみる
        embed.add_field(name="A", value=str(len(A)))
        embed.add_field(name="B", value=str(len(B)))
        embed.set_footer(
            text="made by P4sTela", icon_url="https://raw.githubusercontent.com/P4sTela/P4sTela/main/iconmono_500.png"
        )
