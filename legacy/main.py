...
async def helloworld(inter):
    ...
    emb.title = str(user)
    emb.description = f"**Created at:** `{user.created_at}`\n" f"**ID:** `{user.id}`"
...
@slash.command(
    name="delid",
    description="データベースから情報を削除するよ!",
    options=[Option("id", "変換元の固有id(一つ目の値)", OptionType.INTEGER, required=True)],
    guild_ids=guilds,
)
...
async def vote(ctx):
    row = ActionRow(
        Button(style=ButtonStyle.green, label="A", custom_id="A"), Button(style=ButtonStyle.red, label="B", custom_id="B")
    )
    ...
    async def on_timeout():
        ...
        embed.set_author(name=ctx.author, icon_url=ctx.author.avatar_url)  # Botのアイコンを設定してみる
...
@kaikei.sub_command(description="remove role", options=[Option("user", "remove", OptionType.USER, required=True)])
...
@kaikei.sub_command(description="add role", options=[Option("user", "add", OptionType.USER, required=True)])
